...
Future<Either<Failure, List<PostModel>>> getSavedPosts() async {
  try {
    ...
    if (data == null || data['postsData'] == null) {
      return left(CustomFailure(message: 'لم يتم العثور على بيانات المنشورات المحفوظة'));
    }

    final List<dynamic> postsJson = data['postsData'];
    final List<PostModel> posts = postsJson.map((e) => PostModel.fromJson(e)).toList();
  ...
  @override
  Future<Either<Failure, void>> removePostFromSavedPosts({required String postId})async {
     try {
      final token = getUserData().token;
      await apiConsumer.delete(
